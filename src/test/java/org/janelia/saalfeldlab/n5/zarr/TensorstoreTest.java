...
public class TensorstoreTest {
	...
	@Test
	public void testReadTensorstoreZarr2() throws Exception {
	...
	public void testReadTensorstore(String format) throws Exception {
		...
		//ZarrV3KeyValueWriter n5Zarr = new ZarrV3KeyValueWriter(new FileSystemKeyValueAccess(FileSystems.getDefault()), testZarrDirPath, new GsonBuilder(), false, false, "/", false);
		N5Writer n5Zarr = new ZarrKeyValueWriter(new FileSystemKeyValueAccess(FileSystems.getDefault()), testZarrDirPath, new GsonBuilder(), false, false, "/", false);
		
		String formatGroup = format.equals("--zarr3") ? "zarr3" : format.equals("--zarr2") ? "zarr2" : format.equals("--n5") ? "n5" : null;
		if(formatGroup == null) {
			throw new Exception("Unrecognized format: " + format);
		}
		
		String testZarrGroupName = "tensorstore_tests/" + formatGroup;
		
		/* groups */
		System.out.println("Checking for group: " + testZarrGroupName);
		assertTrue(n5Zarr.exists(testZarrGroupName ) && !n5Zarr.datasetExists(testZarrGroupName));

		/* array parameters */
		final DatasetAttributes datasetAttributesC = n5Zarr.getDatasetAttributes(testZarrGroupName + "/3x2_c_i8");
		...
		final DatasetAttributes datasetAttributesF = n5Zarr.getDatasetAttributes(testZarrGroupName + "/3x2_f_i8");
		...
		assertArrayEquals(
				n5Zarr.getAttribute(testZarrGroupName + "/3x2_c_i8", "dimensions", long[].class),
				new long[]{3, 2});
		assertArrayEquals(
				n5Zarr.getAttribute(testZarrGroupName + "/3x2_c_i8", "blockSize", int[].class),
				new int[]{3, 2});
		assertEquals(
				n5Zarr.getAttribute(testZarrGroupName + "/3x2_c_i8", "dataType", DataType.class),
				DataType.INT64);

		/* LE uint8 in C and F order */
		final UnsignedByteType refUnsignedByte = new UnsignedByteType();

		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/3x2_c_u1"), refUnsignedByte);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/3x2_c_i8"), refLong);
		assertIsSequence(
				Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_i8"), refLong);
		assertIsSequence(
				Views.permute(
					(RandomAccessibleInterval<LongType>)N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_f_i8"),
					0,
					1),
				refLong);
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/3x2_c_u4"), refUnsignedInt);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_u4"), refUnsignedInt);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/3x2_c_f8"), refDouble);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_f8"), refDouble);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/3x2_c_f4"), refFloat);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_f4"), refFloat);
		assertIsSequence(
			Views.permute(
		...
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_u8_zlib"), refUnsignedLong);
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_u8_gzip"), refUnsignedLong);
		assertIsSequence(N5Utils.open(n5Zarr, testZarrGroupName + "/30x20_c_u8_bz2"), refUnsignedLong);
		...
		/* fill value NaN */
		datasetName = testZarrGroupName + "/3x2_c_f4_fnan";
